...
#   NOTE: a 2nd seat on an org already in $ACCOUNT_TAGS MUST be listed here.
#   It shares the first seat's email domain, so the domain fallback below would
#   resolve BOTH profiles to the same tag and the two sessions would be
#   indistinguishable on row 3 - which is exactly why the profile-folder map is
#   consulted first.
...
# .credentials.json at fetch time, and $USAGE_MAXAGE then holds it for 5 minutes.
# So a profile whose identity CHANGES -- a /login as a different account, a second
...
# profile FOLDER while the numbers come from this file, two independent sources that
# disagree silently. Observed 2026-08-11 between two profiles signed in to two seats
# on ONE org: the second profile rendered the FIRST profile's 5h/7d percentages, and
# the first profile's reset times, underneath the second profile's own correct tag.
# The two sides being compared: _email is stamped into the cache from
# /api/oauth/profile using the SAME token that produced the numbers, so it names
# whose numbers these are; .claude.json names who this profile is signed in as NOW.
$script:__profileEmail = $null
function Get-ProfileEmail {
    # Memoized - both the refresh trigger and the render path ask for it.
    if ($null -ne $script:__profileEmail) { return $script:__profileEmail }
    $script:__profileEmail = ''
    $idPath = Join-Path $cfgDir '.claude.json'
    if (Test-Path $idPath) {
        try {
            # .claude.json runs 50-250 KB here, so take the raw text and regex the
            # oauthAccount block rather than ConvertFrom-Json over the whole file.
            $rawId = Get-Content $idPath -Raw
            $i = $rawId.IndexOf('"oauthAccount"')
            if ($i -ge 0) {
                $seg = $rawId.Substring($i, [Math]::Min(2000, $rawId.Length - $i))
                if ($seg -match '"emailAddress"\s*:\s*"([^"]*)"') { $script:__profileEmail = $Matches[1] }
            }
        } catch {}
    }
    $script:__profileEmail
}
function Test-UsageIdentityStale {
    # TRUE only when BOTH identities are known and they differ. An unknown on either
    # side must never hide quota: /api/oauth/profile can fail and leave _email empty,
    # and those numbers are still this profile's own.
    if (-not (Test-Path $usageCache)) { return $false }
    $pe = Get-ProfileEmail
    if (-not $pe) { return $false }
    $ce = ''
    try { $ce = [string](Get-Content $usageCache -Raw | ConvertFrom-Json)._email } catch { return $false }
    if (-not $ce) { return $false }
    ($ce -ne $pe)
}
function Test-UsageRefetchDue {
    (Test-CacheDue $usageCache $USAGE_MAXAGE) -or
    ((Test-UsageIdentityStale) -and (Test-FetchAllowed $usageCache))
}
...
# Allowed = not inside a failure backoff window. Split out of Test-CacheDue so that a
# FORCE-refetch reason (see Test-UsageIdentityStale) can still honour the backoff
# without also having to be past its max age. Bypassing the backoff is exactly what
# pinned the refresh trigger permanently ON in the 2026-08-06 spin bug.
function Test-FetchAllowed($p) { (Get-CacheAge "$p.fail") -ge (Get-FailBackoff $p) }
...
function Invoke-CacheRefresh {
    ...
    if ((Test-UsageRefetchDue) -and (Test-Path $credPath)) {
        try {
...
# The weather arm needs the SAME coordinate guard Invoke-CacheRefresh has at the top of
# its weather block. Without it, an install with blank coords can never write
# $weatherCache, so Test-CacheDue is permanently true, and this trigger spawns a detached
# refresh every 45 s for ever that then does nothing -- the exact spin the negative cache
# was added to kill in 2026-08-06, reachable through the one arm the backoff cannot cover
# (a fetch that is never attempted never records a failure). Fixed 2026-09-09.
if ((($WEATHER_LAT -and $WEATHER_LON) -and (Test-CacheDue $weatherCache $WEATHER_MAXAGE)) -or
    (Test-UsageRefetchDue)                        -or
...
if ($isAnthropic -and (Test-Path $usageCache)) {
    try {
        ...
        # Identity guard: is this cache actually THIS profile's? See
        # Test-UsageIdentityStale. The tag below is still rendered either way -- it
        # comes from the profile folder and stays correct; it is the NUMBERS that
        # would be another account's.
        $identityOk = $true
        $peNow = Get-ProfileEmail
        if ($peNow -and $u._email -and ($u._email -ne $peNow)) { $identityOk = $false }
        # ---- STALENESS GUARD ----
        ...
        # but they must not read as live. Diagnosed 2026-08-22 on cc2: one failed refetch at
        # session start left row 3 printing 8-hour-old percentages at full-strength colors,
        # with both reset arrows pointing at times already PAST, and nothing said so.
        # Two signals, either is enough:
        #   a) a top-level resets_at already in the past -- proof the window has ROLLED, so
        #      these numbers describe a window that has ended. weekly_scoped is excluded on
        #      purpose: its resets_at is null until the scope is used (idle Fable reads 0%).
        #   b) the file older than 2x its max age -- one render cycle behind is the normal
        #      stale-while-revalidate case; twenty means the refetch is failing or nothing
        #      has rendered in hours.
        # Right after a real 5h boundary (a) fires for a render or two before the refetch
        # lands. That is not a false positive: the cached percentage IS the old window's.
        $usageAge   = Get-CacheAge $usageCache
        $usageStale = ($usageAge -gt (2 * $USAGE_MAXAGE))
        foreach ($ra in @($u.five_hour.resets_at, $u.seven_day.resets_at)) {
            if ($ra) { try { if ([datetime]::Parse($ra) -lt (Get-Date)) { $usageStale = $true } } catch {} }
        }
        if ($u._email) {
        ...
        if (-not $identityOk) {
            # Render NOTHING rather than another account's numbers under this tag.
            # Test-UsageRefetchDue has already marked a refetch due (backoff
            # permitting), so this clears within a render cycle or two.
            $partsBottom += "${dim}quota stale$reset"
        }
        ...
        if ($identityOk -and $usageStale) {
            $staleLbl = if ($usageAge -ge 5400) { '{0:0}h' -f ($usageAge / 3600) } else { '{0:0}m' -f ($usageAge / 60) }
            $partsBottom += "${dim}stale $staleLbl$reset"
        }
        if ($identityOk -and $u.five_hour -and $u.five_hour.utilization -ne $null) {
            $bp = [int]$u.five_hour.utilization
            $bc = if ($usageStale) { $dim } elseif ($bp -ge 80) { $red } elseif ($bp -ge 50) { $yellow } else { $green }
            $rt = ''
        ...
        if ($identityOk -and $u.seven_day -and $u.seven_day.utilization -ne $null) {
            $wp = [int]$u.seven_day.utilization
            $wc = if ($usageStale) { $dim } elseif ($wp -ge 80) { $red } elseif ($wp -ge 50) { $yellow } else { $green }
            $wrt = ''
        ...
        foreach ($lim in @($(if ($identityOk) { $u.limits }) | Where-Object { $_.kind -eq 'weekly_scoped' -and $_.scope.model.display_name })) {
            if ($lim.percent -eq $null) { continue }
            $mp = [int]$lim.percent
            $mc = if ($usageStale) { $dim } elseif ($mp -ge 80) { $red } elseif ($mp -ge 50) { $yellow } else { $green }
