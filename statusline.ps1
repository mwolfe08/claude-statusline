    ...
    foreach ($line in $newLines) {
        ...
        if ($line -match '"input_tokens"' -and $line -notmatch '"toolUseResult"') {
            ...
            if ($line -notmatch '"isSidechain"\s*:\s*true') {
                ...
                if     ($line -match '"ephemeral_1h_input_tokens":[1-9]') { $lastTtlKind = '1h' }
                elseif ($line -match '"ephemeral_5m_input_tokens":[1-9]') { $lastTtlKind = '5m' }
            }
        }
    ...
    if ($lastActivity -and $effTtl -gt 0) {
        try {
            ...
            if ($minsLeft -gt 0) {
                ...
                $ttlPart = "${cColor}cache $([int][math]::Ceiling($minsLeft))m$reset"
            } else {
                ...
                $ttlPart = "$tomb cache DEAD $goneStr $reset"
            }
        } catch {}
    }
