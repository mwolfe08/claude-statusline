...
> **Two builds, same output.** `statusline.ps1` is the reference implementation and what
> everything below documents. [`native/`](native/README.md) is a Go port that emits the
> same bytes 8-13x faster - **418 ms -> 33 ms per render** - because most of the cost is
> not the work, it is hosting a 78 KB script in a fresh PowerShell process several times a
> minute in every open session. The two share every cache file and every on-disk format,
> so you can switch between them freely. [`bench/`](bench/README.md) has the measurements,
> including four optimizations tried inside PowerShell that all came to nothing.
...
Two options. **A** is the PowerShell script - no toolchain, edit and go. **B** is the
native build - identical output, far cheaper per render, needs Go once to compile it.

### Option A - the PowerShell script
...
It compiles `native/`, copies the binary to `~/.claude/statusline.exe`, writes
`statusLine.command` in every `settings.json` it finds, and seeds the config file. The
script is left in place, so rollback is one settings key.

**The CONFIG block becomes a config FILE.** A compiled binary cannot carry your
coordinates and account tags, so they live in
`<USERPROFILE>/.claude/statusline-config.json` - see
[`native/statusline-config.sample.json`](native/statusline-config.sample.json) for the
template. The installer seeds it from the CONFIG block of an existing `statusline.ps1`, so
an upgrade keeps your settings. The keys are the same knobs documented below, lowercased
and without the `$`: `weather_lat`, `profile_tags`, `cost_windows`, `cache_ttl_min`, and
so on.

The binary is not committed to this repository - build it yourself, which is also the only
way to be sure of what you are running.
...
- **For the native build only:** Go 1.21+ to compile it once. Nothing beyond the standard
  library - there are no dependencies to audit.
