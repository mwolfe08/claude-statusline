...
The fix is to key each file by the **parent `claude` process PID** instead of `session_id`. One file per terminal, overwritten across `/clear` — so the carried-over cost is counted once. The PID is resolved defensively: `(Get-Process -Id $PID).Parent` on PowerShell 6+, a `Win32_Process` CIM lookup on Windows PowerShell 5.1, and a guard requiring the parent to be named `claude`/`node` so a transient wrapper can never explode the file count. If the process can't be identified it falls back to `session_id` (the old per-session behavior).
